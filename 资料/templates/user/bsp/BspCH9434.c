...
#define BSPCH9434_CS_L gpio_bits_reset(GpioPortDescLib[BoardGpio_CH9434_CS].p_Port, GpioPortDescLib[BoardGpio_CH9434_CS].usPins)
#define BSPCH9434_CS_H gpio_bits_set(GpioPortDescLib[BoardGpio_CH9434_CS].p_Port, GpioPortDescLib[BoardGpio_CH9434_CS].usPins)

static uint8_t ucaBshCH9434_RxBuff[CH9434_RX_BUFF_SIZE];
static uint8_t ucaCH9434Handle[CH9434_CHN_NUM] = {0};
...
static inline void CH9434_US_DELAY(void)
{
  BspDwt_DelayUS(1);
}

static inline void CH9434_MS_DELAY(uint16_t usTime)
{
  BspDwt_DelayMS(usTime);
}

/* SPI交换一个字节接口 */
uint8_t CH9434_SPI_WRITE_BYTE(uint8_t ucByte)
{
  while (spi_i2s_flag_get(SPI3, SPI_I2S_TDBE_FLAG) == RESET)
    ;
  spi_i2s_data_transmit(SPI3, ucByte);
  while (spi_i2s_flag_get(SPI3, SPI_I2S_RDBF_FLAG) == RESET)
    ;
  return (uint8_t)spi_i2s_data_receive(SPI3);
}
...
uint8_t CH9434UARTxFlushRxFIFOData(uint8_t uart_idx, uint16_t read_len)
{
  uint16_t i;
  uint8_t sv_data;
  uint8_t uart_reg_add;
  ...
  for (i = 0; i < read_len; i++)
  {
    ...
    CH9434_US_DELAY();
    CH9434_US_DELAY();
    sv_data = CH9434_SPI_WRITE_BYTE(0xff);
    CH9434_US_DELAY();
    CH9434_SPI_SCS_OP(CH9434_CS_DIS);
  }
...
uint16_t CH9434_Send_Idx_0(uint8_t *p_ucaBuff, uint16_t usLen)
{
  CH9434UARTxSetTxFIFOData(0, p_ucaBuff, usLen);
  return usLen;
}

uint16_t CH9434_Send_Idx_1(uint8_t *p_ucaBuff, uint16_t usLen)
{
  CH9434UARTxSetTxFIFOData(1, p_ucaBuff, usLen);
  return usLen;
}

uint16_t CH9434_Send_Idx_2(uint8_t *p_ucaBuff, uint16_t usLen)
{
  CH9434UARTxSetTxFIFOData(2, p_ucaBuff, usLen);
  return usLen;
}

uint16_t CH9434_Send_Idx_3(uint8_t *p_ucaBuff, uint16_t usLen)
{
  CH9434UARTxSetTxFIFOData(3, p_ucaBuff, usLen);
  return usLen;
}

const BspComPort_Param_S BspComPort_Param_CH9343[CH9434_CHN_NUM] =
    {
        {
            .p_caName = "uart_lcd",
            .p_ucaBuff = &ucaBshCH9434_RxBuff[0 + (0 * CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM)],
            .usSize = (CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM),
            .usTimeOut = 50,
            .p_Send = CH9434_Send_Idx_0,
        },
        {
            .p_caName = "ext_2",
            .p_ucaBuff = &ucaBshCH9434_RxBuff[0 + (1 * CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM)],
            .usSize = (CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM),
            .usTimeOut = 50,
            .p_Send = CH9434_Send_Idx_1,
        },
        {
            .p_caName = "ext_3",
            .p_ucaBuff = &ucaBshCH9434_RxBuff[0 + (2 * CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM)],
            .usSize = (CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM),
            .usTimeOut = 50,
            .p_Send = CH9434_Send_Idx_2,
        },
        {
            .p_caName = "ext_4",
            .p_ucaBuff = &ucaBshCH9434_RxBuff[0 + (3 * CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM)],
            .usSize = (CH9434_RX_BUFF_SIZE / CH9434_CHN_NUM),
            .usTimeOut = 50,
            .p_Send = CH9434_Send_Idx_3,
        },
};

void CH9434_Init(void)
{
  const uint32_t ulaUartBaudrate[CH9434_CHN_NUM] = {115200, 115200, 115200, 115200};
  ...
  /* port 0 init */
  ucaCH9434Handle[CH9434_UART_IDX_0] = BspComPort_Create((BspComPort_Param_S *)&BspComPort_Param_CH9343[CH9434_UART_IDX_0]);

  // config UART para
  CH9434UARTxParaSet(CH9434_UART_IDX_0,                  // port select
                     ulaUartBaudrate[CH9434_UART_IDX_0], // set baud
  ...
  /* port 1init */
  ucaCH9434Handle[CH9434_UART_IDX_1] = BspComPort_Create((BspComPort_Param_S *)&BspComPort_Param_CH9343[CH9434_UART_IDX_1]);

  CH9434UARTxParaSet(CH9434_UART_IDX_1,
                     ulaUartBaudrate[CH9434_UART_IDX_1],
  ...
  /* port 2init */
  ucaCH9434Handle[CH9434_UART_IDX_2] = BspComPort_Create((BspComPort_Param_S *)&BspComPort_Param_CH9343[CH9434_UART_IDX_2]);

  CH9434UARTxParaSet(CH9434_UART_IDX_2,
                     ulaUartBaudrate[CH9434_UART_IDX_2],
                     CH9434_UART_8_BITS_PER_CHAR,
                     CH9434_UART_ONE_STOP_BIT,
                     CH9434_UART_NO_PARITY);
  ...
  /* port 3 init */
  ucaCH9434Handle[CH9434_UART_IDX_3] = BspComPort_Create((BspComPort_Param_S *)&BspComPort_Param_CH9343[CH9434_UART_IDX_3]);

  CH9434UARTxParaSet(CH9434_UART_IDX_3,
                     ulaUartBaudrate[CH9434_UART_IDX_3],
                     CH9434_UART_8_BITS_PER_CHAR,
                     CH9434_UART_ONE_STOP_BIT,
                     CH9434_UART_NO_PARITY);
...
void CH9434_Polling(void)
{
  ...
  for (uint8_t i = 0; i < CH9434_CHN_NUM; i++)
  {
    switch (0x0F & CH9434UARTxReadIIR(i))
    {
    default:
      break;
    ...
    {
      usRxLen = CH9434UARTxGetRxFIFOLen(i);
      if (usRxLen)
      {
        p_ucaLinearBuff = BspComPort_RxBuffGetLinear(ucaCH9434Handle[i], &usLinearBuffSize);
        if ((p_ucaLinearBuff) && (usLinearBuffSize))
        {
          ...
          CH9434UARTxGetRxFIFOData(i, p_ucaLinearBuff, usRxLen);

          BspComPort_RxBuffLinearWriteDone(ucaCH9434Handle[i], usRxLen);

          BspComPort_RxStart(ucaCH9434Handle[i]);
        }
        else
        {
          CH9434UARTxFlushRxFIFOData(i, usRxLen);
        }
      }
    }
    break;
