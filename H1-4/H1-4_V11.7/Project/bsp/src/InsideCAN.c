...
void can_config(void)
{
	...
	i = (setups.idCAN << 21);		   //|CAN_ID_EXT|CAN_RTR_DATA; //SID:11bit[31:21]; EID:19bit[20:3]; EID:b2;RTR:b1;0
	m = (i & 0xE0000000) | 0xE0000000; // 匹配标准ID高三位          //SID:11bit[31:21]; EID:19bit[20:3]; EID:b2;RTR:b1;0
	...
	can_baudrate_struct.rsaw_size = CAN_RSAW_3TQ; // 3TQ;
	can_baudrate_struct.bts1_size = CAN_BTS1_8TQ; // 8TQ;
	can_baudrate_struct.bts2_size = CAN_BTS2_3TQ; // 3TQ;
	...
	can_filter_init_struct.filter_id_high = (uint16_t)(i >> 16);   // SID:11bit[31:21]; EID:19bit[20:3]; EID:b2;RTR:b1;0
	can_filter_init_struct.filter_id_low = (uint16_t)i;			   //(setups.idCAN<<3)|CAN_ID_EXT|CAN_RTR_DATA;
	can_filter_init_struct.filter_mask_high = (uint16_t)(m >> 16); // 匹配标准ID高三位 //SID:11bit[31:21]; EID:19bit[20:3]; EID:b2;RTR:b1;0
	can_filter_init_struct.filter_mask_low = (uint16_t)m;		   // 0x7F8;	//过滤本地ID
...
uint8_t txCAN1(uint8_t *sd, uint16_t num)
{
	...
	for (i = 0; i < num;)
	{
		ts.standard_id = setups.idCAN;					  //<<21;
		ts.extended_id = (setups.idCAN << 8) | target_id; // 本地ID+目标ID
...
void USBFS_L_CAN1_RX0_IRQHandler(void)
{
	if (can_flag_get(CAN1, CAN_RF0MN_FLAG) != RESET)
	{
		...
		i = ts.standard_id;
		d = ts.extended_id;
		if ((i == setups.idCAN) || (d == setups.idCAN) || ((i | d) == 0x00))
		{ // 0x00:广播地址
