...
typedef struct
{
	bool feelback;
	uint8_t send; // 发送使能
	uint8_t rxFIFO[RX_MAX_FIFO];
	uint16_t len;	  // 接收总长度
	uint8_t rxn;	  // 单次接收个数
	uint16_t reTx;	  // 返回字符位置
	uint8_t ep;		  // Enter prompt
	uint16_t TimeOut; // 超时
	uint32_t cto;	  // 计超时数

} rxStructure;

extern rxStructure *tPort, *pUART, *pDUT, *pUSB, *pEther, *pDUT2, *pMultPort1, *pMultPort2, *pMultPort3, *pMultPort4, *pCAN1;
//========================================================================

extern uint8_t socku;
