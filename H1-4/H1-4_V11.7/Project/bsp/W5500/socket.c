  ...
  uint8 ret;
		...
		)
	{
		...
		if (port != 0) {
			 ...
			 ethernet.port++; // if don't set the source port, set local_port number.
			 IINCHIP_WRITE(Sn_PORT0(s) ,(uint8)((ethernet.port & 0xff00) >> 8));
			 IINCHIP_WRITE(Sn_PORT1(s) ,(uint8)(ethernet.port & 0x00ff));
		}
