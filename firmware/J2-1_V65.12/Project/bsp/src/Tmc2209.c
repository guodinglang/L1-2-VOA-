...
void tmc2209_set_run_mode(tmc_run_mode_t mode)
{
    ...
    tmc2209_config.run_mode = mode;
    
    /* 通过UART设置运行模式 */
    uart_data[0] = 0x05;  // 从机地址
    uart_data[1] = 0x00;  // CHOPCONF寄存器地址
    uart_data[2] = 0x00;  // 数据
    uart_data[3] = 0x00;  // 数据
    uart_data[4] = 0x00;  // CRC
    
    tmc2209_uart_send_command(uart_data, 5);
}
...
//		{
//			/* 走多了，反向补偿 */
//			correct_dir = DIR_CW;//(dir == DIR_CW) ? DIR_CCW : DIR_CW;
...
//	tmc2209_move_steps(HOMEING_OFFSET, MOVE_SPEED, DIR_CCW); // 先往反方向移动20000微步，保证离开归零位

	tmc2209_move_steps(HOMEING_OFFSET, HOMEDING_SPEED, DIR_CW); // 先往反方向移动20000微步，保证离开归零位

	g_origin_location_flag = 0;
    pin_states.org_state   = 0;

	tmc2209_set_direction(DIR_CCW);// DIR_CCW为归零方向
    ...
    while(!pin_states.org_state) //第一次归零  粗归零  编码器值0~25
    {
        ...
        /* 发送数据 */
        delay_us = 1000000 / HOMEDING_SPEED_1;
        for(i = 0; i < delay_us; i++) {
            __NOP();
        }
        timeout++;
        if(timeout > 150000) {  // 超时
           motor_status.homing = 0;
    ...
    /* 等待发送缓冲区空 */
    timeout=0;
    while(!pin_states.org_state) //第二次归零  慢速归零
    {
        tmc2209_step_pulse(); 

        delay_us = 1000000 / (100);
        for(i = 0; i < delay_us; i++) {
            __NOP();
        }
        timeout++;
...
//	{M_SPREAD_PORT->clr = M_SPREAD_PIN;}
    delay_ms(10);
    tmc2209_uart_write_reg(TMC2209_REG_GCONF,0x00000092);//92
    delay_ms(10);
    tmc2209_uart_write_reg(TMC2209_REG_GCONF,0x00000092);//92    
    delay_ms(10);
    tmc2209_set_current(400, 50);      // 720mA运行电流，80%保持电流
    delay_ms(10);
...
//    if (forward)
//    {
//        tmc2209_set_direction(DIR_CCW);// 逆时针
//        //printf("DIR_CCW\r\n");
//    }    
//    else
//    {
//        tmc2209_set_direction(DIR_CW);//DIR_CW
//        //printf("DIR_CW\r\n");
//    }
//}
/***************************************************************************
...
//	ret = tmc2209_homing(HOMEDING_SPEED);
