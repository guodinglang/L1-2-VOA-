...
#define DAC_POWER(n)   n>0? (GPIOC->scr = GPIO_PINS_13):(GPIOC->clr = GPIO_PINS_13)    //
...
#define DAC_CLR(n)       n>0? (GPIOB->scr = GPIO_PINS_1):(GPIOB->clr = GPIO_PINS_1)	 // ?½???????J?????????????0V;?????H
#define DAC_CS(n)        n>0? (GPIOB->scr = GPIO_PINS_0):(GPIOB->clr = GPIO_PINS_0)	 //
#define DAC_LDAC(n)      n>0? (GPIOA->scr = GPIO_PINS_4):(GPIOA->clr = GPIO_PINS_4)  // 0=?????J???????????1=??????????
