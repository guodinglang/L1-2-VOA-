...
{  
	uint8_t i,len=0,buff[64]={0};

	switch(getSn_SR(SOCK_TCPS))                             	// 获取socket的状态
	{
		case SOCK_CLOSED:                                     	// socket处于关闭状态
			setKPALVTR(SOCK_TCPS,1);                            // KEEP ALIVE配置(需由W5500主动发送过一次数据后生效)
			socket(SOCK_TCPS ,Sn_MR_TCP,ethernet.port,Sn_MR_ND);   // 打开socket
		break;     
		...
		case SOCK_ESTABLISHED:                                	// socket处于连接建立状态
			...
			if(len>0)
			{
				...
				for(i=0;i<len;i++)
				{
					...
					{
						pEther->rxn=3;
						pEther->rxFIFO[pEther->len++]=buff[i];
					}
					else if(buff[i] == 0x08)
					{
						if(pEther->len >0)
						{
							pEther->len--;
							pEther->rxFIFO[pEther->len]=0;
						}
					}
					else if(pEther->len < RX_MAX_FIFO)
					{pEther->rxFIFO[pEther->len++]=buff[i];}	
//					if(pEther->len < RX_MAX_FIFO)
//					{pEther->rxFIFO[pEther->len++]=buff[i];}
				}
