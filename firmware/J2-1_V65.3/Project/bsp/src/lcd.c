		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont16[k].Msk[i]&(0x01<<j))LCD_WR_wDATA(fc);
						else LCD_WR_wDATA(bc);
					}
					else//叠加方式
					{
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont24[k].Msk[i]&(0x01<<j))LCD_WR_wDATA(fc);
						else LCD_WR_wDATA(bc);
					}
					else//叠加方式
					{
		...
		{ 	
			...
			for(i=0;i<TypefaceNum;i++)
			{
				...
				{	
					if(!mode)//非叠加方式
					{
						if(tfont32[k].Msk[i]&(0x01<<j))LCD_WR_wDATA(fc);
						else LCD_WR_wDATA(bc);
					}
					else//叠加方式
					{
...
void LCD_ShowChar(u16 x,u16 y,u8 num,u16 fc,u16 bc,u8 sizey,u8 mode)
{
	...
	for(i=0;i<TypefaceNum;i++)
	{ 
		if(sizey==16)temp=ascii_08X16[num][i];		       //调用8x16字体
		else if(sizey==32)temp=ascii_16X32[num][i];		 //调用16x32字体
		else return;
		for(t=0;t<8;t++)
		{
			if(!mode)//非叠加模式
			{
				if(temp&(0x01<<t))LCD_WR_wDATA(fc);
				else LCD_WR_wDATA(bc);
			}
			else//叠加模式
			{
	...
	while(len--)
	{
		...
		{ 
			...
			for(t=0;t<8;t++)
			{
				...
				{
					if(b&(0x01<<t)){LCD_DrawPoint(x,y,fc);}//画一个点
					x++;
					if((x-x0)==sx)
					{
						x=x0;
...
void lcd_init(void)
{
	...
	if(USE_HORIZONTAL==0)LCD_WR_bDATA(0x00);
	else if(USE_HORIZONTAL==1)LCD_WR_bDATA(0xC0);
	else if(USE_HORIZONTAL==2)LCD_WR_bDATA(0x70);
	else LCD_WR_bDATA(0xA0);
